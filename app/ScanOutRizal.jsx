...
function ScanOutRizalScreen() {
  ...
  console.log('ParkingContext in ScanOutRizalScreen:', { decrementCount });
  ...
  async function handleScan(data) {
    ...
    if (!student_number || parkinglocname !== 'Rizal' || !vehicle?.plate_number) {
      ...
      Alert.alert('Error', 'QR code not valid for Rizal scan out');
      lockScanner();
      return;
    }
    ...
    try {
      ...
      if (transactionError || !transaction) {
        ...
        throw new Error('No active parking session found for this user at Rizal');
      }
      ...
      if (slotData.current_occupancy <= 0) {
        throw new Error('No occupied slots to decrement in Rizal');
      }
